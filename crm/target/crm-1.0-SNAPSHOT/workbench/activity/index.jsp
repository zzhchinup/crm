...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#addBtn").on("click",function (){
			...
			$.ajax({
				...
				success : function (data){
					 ...
					 */
					var html ="";
					// 遍历出来的每一个n就是每一个user对象，i是位置
					$.each(data,function (i,n){
						html +="<option value='"+n.id+"'>"+n.name+"</option>";
					})

					$("#create-owner").html(html);
					// 将当前登录的用户设置为下拉框设置的默认选项
					var id = "${user.id}";
					$("#create-owner").val(id);
					// 所有者下拉框处理完毕之后，处理模态窗口
			...
			if($xz.length==0){
				...
				$.ajax({
					...
					success : function (data){
						...
						$("#edit-description").val(data.activityInfo.description);
						$("#edit-cost").val(data.activityInfo.cost);
						$("#edit-id").val(data.activityInfo.id);

						// 所有的值都处理好之后，打开修改操作的模态窗口
		...
		$("#activityBody").on("click",$("input[name=xz]"),function (){
			$("#selectAllBox").prop("checked",$("input[name=xz]").length==$("input[name=xz]:checked").length);
		})

		// 为删除按钮绑定事件，执行时长活动的删除操作
		$("#deleteBtn").on("click",function (){
			...
			if($xz.length==0){
				...
				if(confirm("确定删除所选中的记录吗？")){
					...
					// 将$.xz中的每一个dom对象遍历出来，就相当于取出了需要删除的记录的id
	...
	/*
		...
		我们都在哪些情况下需要调用pageList方法，也就是什么情况下需要刷新一下列表呢？
			(1)点击左侧"市场活动的时候"
			(2)保存，修改，删除的时候。
			(3)点击查询的时候。
			(4)点击分页的组件的时候。
					...
					<%--
						...
						点击创建按钮，观察两个属性和属性值
							...
							data-target="#createActivityModal"
								表示要打开哪个模态窗口，通过#id的形式打开模态窗口。

							现在是通过属性和属性值的方式写在了button元素中，打开模态窗口，
							但是这样做是有问题的：
								如果说在打开窗口的时候，同时处理其他的事情，是没有办法的，也就是说功能不能扩充。

							所以未来的实际项目开发，对于模态窗口的操作，一定不要写死在元素中
							应该我们自己写在js中。
					--%>
